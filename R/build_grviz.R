#' Build stard diagram
#'
#' Build a \code{grob} stard diagram, use this if you want
#' to save plots with \code{\link[ggplot2]{ggsave}}
#'
#' @param x A stard object.
#' 
...
build_grviz <- function(x) {
  ...
  if (!inherits(x, c("stard")))
    stop("x must be stard object")
  ...
  if(any(grepl("label", names(x)))){
    stard_plot <- x[grepl("node", names(x))]
  ...
  nd_type <- sapply(nodes_layout, function(x) 
    unique(sapply(stard_plot[x], "[[", "node_type"))
  )
  
  if(nd_type[length(nd_type)] == "sidebox")
    stop("last box cannot be a side box.")
  
  main_txt <- lapply(names(stard_plot), function(nd){
    nd_num <- gsub("\\D", "", nd)
    txt_lab <- mk_text_align(stard_plot[[nd]]$text, stard_plot[[nd]]$just)
    c(nam = nd,
  ...
  for(i in seq_along(nd_type)){
    ...
    if(length(nodes_layout[[i+1]]) == length(nd) & all(!c(nd_type[i], nd_type[i+1]) %in% "sidebox")){
      ...
      for(j in seq_along(nd)){
        ...
        if(current_nd %in% nodes_connect & !is_empty(stard_plot[[current_nd]]$text)){
          connect_to <- max(which(nodes_connect == current_nd))
          connect_to <- names(nodes_connect[connect_to])
          if(!is_empty(stard_plot[[connect_to]]$text))
    ...
    if(nd_type[i] == "sidebox"){
      ...
      if(length(nd) == 1){
        # Strip white space
        if(is_empty(stard_plot[[nd]]$text)){
          con_nd <- c(con_nd, sprintf("%s -> %s;", 
                                      nodes_layout[[i-1]], 
                                      nodes_layout[[i+1]]))
        }else{
        ...
        for(j in seq_along(nd)){
          ...
          if(is_empty(stard_plot[[nd[j]]]$text)){
            ...
            if(!is_empty(stard_plot[[nd_next]]$text)){
              con_nd <- c(con_nd, sprintf("%s -> %s;", 
                                          nodes_layout[[i-1]][j],
                                          nd_next))
            }
  ...
  if(any(!grepl(paste(nodes_connect, collapse = "|"), con_nd))){
    ...
    con_nd_extra <- sapply(seq_along(nd_none), function(idx){
      ...
      if(stard_plot[[con_to]]$node_type == "sidebox"){
        return("")
      }else{
        if(!is_empty(stard_plot[[con_to]]$text))
  ...
  grviz_txt <- paste("digraph stard_diagram {
  graph [layout = dot]",
  lab_nd,
  lab_edge,
