...
#' # g <- ggsave("stard_diagram.pdf", plot = build_grid(g))
#' }
#' 
build_grid <- function(x) {
  ...
  if(any(grepl("label", names(x)))){
    ...
    attr(stard_plot, "nodes.list") <- attr(x, "nodes.list")
    label_plot <- x[grepl("label", names(x))]
  }else{
    stard_plot <- x
  }
  
  node_tp <- which(sapply(stard_plot, "[[", "node_type")  == "splitbox")
  if(!all(abs(diff(node_tp)) == 1))
    stop("Multiple splits are not supported, use `grViz` method instead. See `build_grviz` for details.")
  
  # Coordination
  nodes_coord <- calc_coords(stard_plot)
  
  # Generate connection
  nodes_connect <- get_connect(stard_plot)
  ...
  nodes <- sapply(names(stard_plot), function(i){
    r <- move_box(stard_plot[[i]]$box,
                  x = unit(nodes_coord$x[i], "npc"),
                  y = unit(nodes_coord$y[i], "npc"))
    r$name <- i
    
    # Skep empty side box
    if(is_empty(stard_plot[[i]]$text))
      return(NULL)
