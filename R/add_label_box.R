...
#'
#' In a stard diagram, this can be used to indicate different stage.
#'
...
add_label_box <- function(prev_box,
  ...
  if (!inherits(prev_box, c("stard"))) {
    stop("prev_box must be stard object")
  }
  ...
  args_list <- list()
  # args_list$text <- txt
  args_list$txt_gp <- gpar(col = col, cex = cex, fontface = "bold", fontfamily = fontfam)
  args_list$box_gp <- gpar(fill = fill)
  ...
  class(node_list) <- union("stard", class(node_list))
