...
calc_coords <- function(stard_plot){
  
  # Get the maximum of height and width of each node
  nodes_layout <- attr(stard_plot, "nodes.list")
  ...
  for(i in seq_along(nodes_layout)){
    nd <- sapply(stard_plot[nodes_layout[[i]]], function(x)
  ...
  nd_wd <- lapply(nodes_layout, function(nd){
    sapply(stard_plot[nd], function(x){
  ...
  nd_sides <- lapply(nodes_layout, function(nd){
    unlist(sapply(stard_plot[nd], function(nd)nd$side))
  })
